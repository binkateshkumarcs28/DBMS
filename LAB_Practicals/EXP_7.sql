...
SELECT MAX(SAL) AS MAXIMUM_SALARY,
        MIN(SAL) AS MINIMUM_SALARY,
         MAX(SAL) - MIN(SAL) AS SALARY_DIFF FROM employee;
...
SELECT CONCAT('$',SAL/91) AS SALARY_IN_DOLLER FROM employee;

-- 5. Create a matrix query to display the job, the salary for that job based on department number, and the total salary for that job
...
SUM(SAL) AS TOTAL_SALARY_EACH_DEPARTMENT FROM EMPLOYEE GROUP BY JOB;
...
SELECT YEAR(HIREDATE), COUNT(*) AS TOTAL_EMPLOYEE_OF_THAT_YEAR FROM employee GROUP BY YEAR(HIREDATE);

-- 7. Query to get the last Sunday of Any Month.

 SELECT DATE_SUB(
                LAST_DAY(CURDATE()),
                     INTERVAL(DAYOFWEEK(LAST_DAY(CURDATE())) - 1) DAY) LAST_SUNDAY;

-- 8. Display department numbers and total number of employees working in each department.

SELECT DEPTNO, COUNT(*) AS TOTAL_employee_EACH_DEPARTMENT FROM employee GROUP BY DEPTNO;

-- 9. Display the various jobs and total number of employees within each job group.

SELECT JOB, COUNT(*) AS TOTAL_employee_EACH_DEPARTMENT FROM employee GROUP BY JOB;

-- 10. Display the department numbers and total salary for each department.

SELECT DEPTNO, SUM(SAL) AS TOTAL_SALARY_EACH_DEPARTMENT FROM employee GROUP BY DEPTNO;
